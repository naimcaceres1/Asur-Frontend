...
type TimeRange = "90d" | "30d" | "7d" | "custom";
type Metric = "RESERVAS" | "INSCRIPCIONES";
...
type MisSerieResponse = {
    reservas: MisSeriePoint[];
    inscripciones: MisSeriePoint[];
};
...
function getChartTheme(activeTheme: string) {
...
export function DashboardMisReservasInscripcionesChartClient() {
    ...
    const userId = (session as any)?.userData?.extra?.idUsuario ?? null;

    const [timeRange, setTimeRange] = useState<TimeRange>("90d");
    const [metric, setMetric] = useState<Metric>("RESERVAS");
    const [chartStyle, setChartStyle] = useState<ChartStyle>("area");
    const [loading, setLoading] = useState(false);
    const [usageSeries, setUsageSeries] = useState<MisPoint[]>([]);
    ...
    const chartConfig: ChartConfig = useMemo(() => {
        ...
        const theme = getChartTheme(activeTheme || "default");
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            try {
                ...
                params.set("idUsuario", String(userId));

                const raw: MisSerieResponse = await apiClient(
                    `/graficos/mis-reservas-inscripciones?${params.toString()}`,
                    token
                );

                const merged = new Map<string, MisPoint>();

                (raw.reservas ?? []).forEach((p) => {
                    const key = p.fecha;
                    merged.set(key, {
                        fecha: key,
                        reservas: p.cantidad ?? 0,
                        inscripciones: 0,
                    });
                });
                ...
                console.error(
                    "Error obteniendo datos de gráficos (mis reservas/inscripciones):",
                    error
                );
        ...
        fetchData();
    }, [token, userId, timeRange, calendarRange]);

    const chartData: ChartPoint[] = useMemo(() => {
        return usageSeries.map((p) => ({
            date: p.fecha,
            value: metric === "RESERVAS" ? p.reservas ?? 0 : p.inscripciones ?? 0,
        }));
    }, [usageSeries, metric]);
            ...
            <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
                ...
                {!loading && !hasAnyValue && (
                    <p className="mt-2 px-2 text-xs text-muted-foreground">
                        No hay coincidencias para la búsqueda seleccionada.
                    </p>
                )}
            </CardContent>
        </Card>
